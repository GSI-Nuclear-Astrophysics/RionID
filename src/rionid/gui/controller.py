...
from numpy import append, argsort, where
...
def import_controller(
    ...
    try:
        # initializations
        if float(alphap) > 1:
            alphap = 1 / float(alphap) ** 2  # handling alphap and gammat
        ...
        logger.info(
            "Simulation results (ordered by frequency) will be saved to simulation_result.out"
        )
